...
function timedown(ti, id) {
  ...
  const x = setInterval(function () {
    ...
    else if (now < countDownDate + 86400000) {
      // 86400000 is the number of milliseconds in one day
      let subtitleString = `🎉 Happy New Year ${currentYear} ! :D`
      document.getElementById("subtitle").innerHTML = subtitleString;
      document.getElementById(id).innerHTML = subtitleString;
      document.getElementById("subtitleMaybe").innerHTML =  '2024' + '<br>' + (days != 0 ? days + " D " : "") +
      (hours != 0 ? hours + " H " : "") +
      (minutes != 0 ? minutes + " M " : "") +
      (seconds + "S");
      var cDuration = 5 * 1000;
